...
# 3) Keep student_id for traceability in output
...
# - final_score: target
# - passed: derived from target (leakage for regression)
...
# 9) Standard regression metrics
mae = mean_absolute_error(y_test, pred)
rmse = mean_squared_error(y_test, pred) ** 0.5
r2 = r2_score(y_test, pred)

# 10) Per-row output + relative error (%)
out = pd.DataFrame({
...
out["abs_pct_error"] = np.where(
    out["y_true"] != 0,
    np.abs(out["error"] / out["y_true"]) * 100.0,
    np.nan
)

# 11) Error tendency summaries
